...
const placeOrder = async (req, res) => {
  try {
    ...
    const cart = await Cart.findOne({ userId }).populate({
      ...
      populate: { path: 'brand category offer', model: 'Brand' }
    });
    ...
    for (const item of cart.items) {
      ...
      let bestDiscount = product.offer && product.offer.status === 'active' ? product.offer.discount : 0;
      let discountedPrice = Math.round(originalPrice * (1 - bestDiscount / 100));
    ...
    if (paymentMethod === 'Razorpay') {
      try {
        ...
        savedOrder = await newOrder.save(); // Save with 'Pending' status if Razorpay fails or is canceled
...
const orderSummary = async (req, res) => {
  try {
    ...
    // Check if it's a pending Razorpay payment
